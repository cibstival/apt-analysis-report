...
### 실거래 데이터
기본은 국토부 실거래가 공개시스템(rt.molit.go.kr)에서 **키 없이** 직접 내려받습니다(`scripts/fetch_rtms_direct.py`, 동·층 포함). 사이트 구조가 바뀌어 실패할 때만 아래 API 키를 씁니다.
...
```
```
잠실엘스 33평 가지고 있는데 지금 27억에 팔아도 될까?
```
단지명·거래가는 필수이고, 동·호·평형·매수/매도·실거주 여부는 있으면 정확해집니다. 빠진 정보는 Claude가 **한 번에 모아서** 묻고, "그냥 만들어줘"라고 하면 최근 최고 실거래를 기준가로 써서 진행합니다. 동을 알려주면 보고서에 **같은 동 실거래 최고가·평균 대비** 비교가 들어갑니다.
```
