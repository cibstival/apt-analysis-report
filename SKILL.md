...
# 아파트 살까말까 (apt-saramamra) — 단지·물건 분석 보고서 생성

## 호출 시 첫 동작
- **`/apt-saramamra` 만 입력(인자 없음)** → 조사·검색을 시작하기 전에 **즉시** 아래 인테이크 질문을 한 번에 던진다. 인사말이나 스킬 설명은 붙이지 않는다.
  ...
  ```
- **`/apt-saramamra 홍은현대 101동 1102호 8.4억 매수`** 처럼 인자가 있으면 0-b 규칙으로 파싱하고, 필수 항목(단지·거래가)이 빠졌을 때만 위 질문 중 해당 항목을 묻는다.
